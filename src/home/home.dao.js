...
// 회원이면 회원이 설정한 카테고리 우선정렬, 비회원이면 전체 카테고리 가나다순 정렬해서 반환
export const findUserCategoriesById = async(userId) => {
    try {
        const conn = await pool.getConnection();
        ...
        let userCategories = [];
        if (userId) {
            [userCategories] = await conn.query(getUserCategories, [userId]);
        }

        // 카테고리가 없거나 비회원인 경우 전체 카테고리 조회(가나다순)
        if (userCategories.length === 0) {
            [userCategories] = await conn.query(getAllCategories);
        }
...
// findUserRecommendShortsById

// findUserFollowingFeedList
